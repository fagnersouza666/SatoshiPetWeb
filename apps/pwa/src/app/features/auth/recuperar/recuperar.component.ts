...
@Component({
  ...
  template: `
    <section class="page" aria-labelledby="recuperar-titulo">
      ...
      @if (!sucesso()) {
        <form [formGroup]="form" (ngSubmit)="recuperar()" novalidate aria-label="Formulário de recuperação de acesso">
  ...
  styles: [`
    ...
    .alert {
      ...
      &__icon { flex-shrink: 0; margin-top: 0.1rem; }
    }
  ...
  protected async recuperar(): Promise<void> {
    ...
    try {
      await firstValueFrom(
        this.api.post<void>('/v1/auth/recover', { recoveryCode }),
      );
